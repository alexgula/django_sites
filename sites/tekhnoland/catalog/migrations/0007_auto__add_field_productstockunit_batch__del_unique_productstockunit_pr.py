...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'ProductStockUnit', fields ['product', 'pending', 'stock']
        db.delete_unique('catalog_productstockunit', ['product_id', 'pending', 'stock'])

        # Adding field 'ProductStockUnit.batch'
        db.add_column('catalog_productstockunit', 'batch', self.gf('django.db.models.fields.CharField')(max_length=36, null=True, blank=True), keep_default=False)

        # Adding unique constraint on 'ProductStockUnit', fields ['product', 'batch', 'stock']
        db.create_unique('catalog_productstockunit', ['product_id', 'batch', 'stock'])

    def backwards(self, orm):

        # Removing unique constraint on 'ProductStockUnit', fields ['product', 'batch', 'stock']
        db.delete_unique('catalog_productstockunit', ['product_id', 'batch', 'stock'])

        # Deleting field 'ProductStockUnit.batch'
        db.delete_column('catalog_productstockunit', 'batch')
